...
| 仿真桥接 | `SimulationBridge`；`WUTA-SIM/simulator_bringup/simulator_bringup/simulation_bridge.py` | 发布就绪/任务状态；真值 pose/TF 仅作显式回退 |
...
过滤。自交叉 Skidpad 路径通过单调前向进度索引保持圈序；停车后控制器通知 bridge 发布
`FINISH`。
